...
import { StyleSheet, View, Text, TouchableOpacity, TextInput, Alert, Pressable } from "react-native";
...
const ResetPassword = () => {
	...
	const handleUpdatePassword = async () => {
		...
		if (password !== confirmPassword) {
			Alert.alert("Match Error", "Passwords do not match.");
			return;
		}

		if (!isAtLeast8Chars || !hasUppercase || !hasNumber || !hasSpecialChar) {
			Alert.alert("Security Check", "Please satisfy all password complexity rules.");
			return;
		}
		...
		try {
			...
			Alert.alert("Success 🚀", "Your password update request has been securely submitted.", [
				{ text: "OK", onPress: () => router.replace("/(auth)/sign-in") }
			]);
		} catch (error: any) {
			console.log("🔴 Reset Password Transaction Error:", error);
			Alert.alert("Update Failed", error.message || "An unexpected credential routing exception occurred.");
		} finally {
			setLoading(false);
		}
	};
